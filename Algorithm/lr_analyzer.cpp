...
void LRAnalyzer::constructActionGotoTable(bool isLR1) {
    ...
    for (size_t i = 0; i < itemSets.size(); ++i) {
        for (const auto& item : itemSets[i].items) {            if (!item.isReduceItem()) {
                // 处理移进和GOTO动作
                std::string nextSymbol = item.getNextSymbol();
...
void LRAnalyzer::constructLR1ItemSets() {
    ...
    while (!workList.empty()) {
        ...
        // 对每个符号计算GOTO
        for (const auto& symbol : symbols) {
            ItemSet gotoResult = gotoSetLR1(itemSets[currentIndex], symbol);
